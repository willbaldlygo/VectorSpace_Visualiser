...
composer.addPass(bloomPass); // RE-ENABLED
...
nebulaTexture.wrapS = THREE.RepeatWrapping; // Fix seam
...
const bgMat = new THREE.PointsMaterial({
  size: 2,
  vertexColors: true,
  map: starTexture, // Fix square stars
  transparent: true,
  opacity: 0.6,
  sizeAttenuation: true,
  depthWrite: false
...
const SEQUENCE_COLORS = [
  [0.35, 0.78, 1.0],  // Vibrant Pastel Blue
  [0.55, 0.85, 0.45], // Vibrant Pastel Green
  [1.0, 0.65, 0.5],   // Vibrant Pastel Orange
  [1.0, 1.0, 0.5],    // Vibrant Pastel Yellow 
  [0.85, 0.5, 0.82],  // Vibrant Pastel Pink
  [0.0, 0.85, 0.75]   // Vibrant Pastel Teal
];
...
function updateVisualization(words) {
  ...
  for (let w = 0; w < wordCount; w++) {
    ...
    const spriteMaterial = new THREE.SpriteMaterial({
      ...
      transparent: true,
      opacity: 0.8, // Slightly lower opacity for larger glow
      blending: THREE.AdditiveBlending,
      depthWrite: false
    });
    const sprite = new THREE.Sprite(spriteMaterial);
    sprite.position.copy(pos);
    sprite.scale.set(150, 150, 1); // Increased from 60 to 150
    scene.add(sprite);
    activeSprites.push(sprite);

    // KNN Cloud (Linked Stars)
    ...
    neighbors.forEach(n => {
      ...
      const intensity = 1 - (dist / maxDist); // Linear falloff for wider cloud

      // Higher multiplier (12.0) for neighborhood stars to make the link very direct
      colorsAttr[i * 3] = Math.max(colorsAttr[i * 3], col[0] * intensity * 12.0);
      colorsAttr[i * 3 + 1] = Math.max(colorsAttr[i * 3 + 1], col[1] * intensity * 12.0);
      colorsAttr[i * 3 + 2] = Math.max(colorsAttr[i * 3 + 2], col[2] * intensity * 12.0);

      sizesAttr[i] = Math.max(sizesAttr[i], 6.0 + (intensity * 10.0)); // Larger stars for better presence
    });
  }
  ...
  if (wordCount >= 6) {
    ...
    // statusEl.style.color = "#00ffff"; // Removed to use CSS Ivory
...
window.addEventListener('resize', () => {
  const sidebarWidth = document.getElementById('controls').classList.contains('minimized') ? 40 : 300;
